...
const SideBar = ({
  ...
  const handleLogout = () => {
    ...
    setTimeout(async () => {
      await simulatedWallet.disconnect();
  ...
  return (
    <div
      ...
      } w-64 h-full bg-white/5 backdrop-blur-xl border-r border-white/10 shadow-2xl shadow-indigo-500/20 flex flex-col overflow-y-auto`}
    >
      <div className="flex items-center gap-3 px-4 py-6 border-b border-white/10 shrink-0">
        <App_logo />
        ...
        {menuItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 shrink-0 ${
                isActive
            ...
            >
              ...
              {isActive && (
                <span className="ml-auto w-1.5 h-1.5 rounded-full bg-indigo-400 animate-pulse shrink-0"></span>
              )}
            </button>
          );
        })}
      ...
      <div className="border-t border-white/10 p-4 space-y-3 shrink-0">
        <div className="flex items-center gap-3 px-2">
          <div className="w-10 h-10 min-w-10 rounded-full bg-gradient-to-r from-indigo-500 to-purple-500 flex items-center justify-center text-white font-bold text-sm">
            {initial}
          </div>
