...
export const getBalance = async (
  ...
  if (!xrpl.isValidAddress(address)) {
    throw new Error(
      "Dirección XRPL inválida."
    );
  }

  const client =
    new xrpl.Client(XRPL_TESTNET);
  ...
  try {
    ...
    console.log(
      "⏳ Consultando balance XRP..."
    );

    await client.connect();

    console.log(
      "✅ Conectado a XRPL Testnet"
    );
